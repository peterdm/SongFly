...
$.prettyDate = {
	...
	format: function(time) {
		var date = new Date((time || "").replace(/\.\d{3}/,"").replace(/-/g,"/").replace(/[TZ]/g," ")),
			diff = ($.prettyDate.now().getTime() - date.getTime() + ($.prettyDate.now().getTimezoneOffset() * 60000)) / 1000,
		...
		return day_diff == 0 && (
				...
				diff < 240 && messages.nowlistening ||
				diff < 1800 && messages.minutes(Math.floor( diff / 60 )) ||
				diff < 3600 && messages.withinhour ||
